...
import '../../providers/routine_notifier.dart';
...
class _RoutineScreenState extends ConsumerState<RoutineScreen>
  ...
  void initState() {
    ...
    // Affiche le popup de badges quand le notifier en signale de nouveaux.
    ref.listenManual<AsyncValue<RoutineData>>(routineProvider, (prev, next) {
      final badges = next.value?.nouveauxBadges ?? const [];
      if (badges.isNotEmpty) {
        _showBadgeUnlock(badges);
        ref.read(routineProvider.notifier).acquitterNouveauxBadges();
      }
    });
  }
  ...
  String _messageMotivation(double progression) {
    if (progression >= 1.0) return '🎉 Parfait ! Toutes les tâches sont faites !';
    if (progression >= 0.75) return '💪 Encore un petit effort, c\'est presque fini !';
    if (progression >= 0.50) return '👍 Bonne progression, continuez !';
    if (progression >= 0.25) return '🌱 C\'est parti, bon début !';
      ...
      appBar: CuAppBar(
        ...
        extraActions: [
          ...
          IconButton(
            ...
            tooltip: 'Mes badges',
            onPressed: () => _showBadges(routineAsync.value?.badges ?? const []),
          ),
        ],
      ),
        ...
        data: (data) => RefreshIndicator(
          ...
          child: ListView(
            ...
            children: [
              ...
              _StreakCard(profil: data.profil),
              const SizedBox(height: 12),

              // ── Barre de progression du jour ──
              _ProgressCard(
                nbCompletees: data.nbCompletees,
                total: data.taches.length,
                scoreActuel: data.scoreActuel,
              ...
              _MotivationBanner(
                progression: data.progression,
                message: _messageMotivation(data.progression),
              ),
              const SizedBox(height: 16),
              ...
              if (data.taches.isEmpty)
                ...
                ...data.taches.map((t) => _TacheItem(
                      tache: t,
                      estFait: data.completees.contains(t.id),
                      onToggle: () => notifier.toggleTache(t),
                    )),
  ...
  void _showBadges(List<BadgeEleveur> badges) {
    final obtenus = badges;
      ...
      builder: (_) => DraggableScrollableSheet(
        ...
        builder: (_, scrollController) => ListView(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              '${obtenus.length} / ${BadgeEleveur.tousLesBadges.length} débloqués',
              textAlign: TextAlign.center,
              style: TextStyle(color: context.cuTextSecondary),
            ),
            const SizedBox(height: 16),
            ...BadgeEleveur.tousLesBadges.map((bd) {
              final obtenu = obtenus.any((b) => b.code == bd.code);
              return ListTile(
                leading: Text(
                  obtenu ? bd.icone : '🔒',
  ...
  Future<void> _ajouterTache() async {
    ...
    if (ok == true && titreCtrl.text.trim().isNotEmpty) {
      await ref.read(routineProvider.notifier).ajouterTache(Tache(
